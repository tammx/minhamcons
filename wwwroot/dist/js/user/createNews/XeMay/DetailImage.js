$(document).ready(function () {
    ...
    $('.next').click(function () {
        ...
        var video = $('#Video').val();
        if (hinhanhchitiet) {
            var soHinh = (hinhanhchitiet.split('|').filter(x => x)).length;
            if (soHinh > 2 && checkVideoValid(video)) {
                $('.next-form').submit();
            }
            else if (soHinh < 3) {
                $.notify({
                    message: 'Vui lòng upload tối thiểu 3 hình ảnh xe để tiếp tục!'
                }, {
                    type: 'danger'
                });
            }
