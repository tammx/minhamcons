    ...
    var myDropzone = new Dropzone("#myDropzone", {
        ...
        init: function () {
            ...
            self.on("addedfile", function (file) {
                ...
                if (imgs && imgs.split('|').filter(x => x).length > 2) {
                    $.notify({
                        message: 'Bạn chỉ được upload tối đa 2 hình ảnh cavet!'
                    }, {
                        type: 'danger'
                    });
                }
...
function removeImg(fileName) {
    ...
    var imgVal = $('#img-container').val();
    $.ajax({
        url: '@Url.Action("DeleteImage", "Sys")' + '?fileName=' + fileName + '&path=cavet',
        processData: false,
        async: false,
        contentType: false,
        type: "POST",
